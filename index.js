...
function warptext(paragraph, maxCharsPerLine) {
        ...
        while (paragraph.length > maxCharsPerLine) {
                ...
                if (paragraph.charAt(maxCharsPerLine + 1) == " ") {
                        lines.push(line);
                        paragraph = paragraph.substr(maxCharsPerLine + 1).trim();

                } else {
                        let lastSpaceIndex = line.lastIndexOf(" ");

                        // For when the word is one piece , without space  
                        if (lastSpaceIndex == -1) {
                                line = line.substr(0, maxCharsPerLine - 1);
                                line += '-';
                                lines.push(line);
                                paragraph = paragraph.substr(maxCharsPerLine + 1).trim();
                        } else {
                                line = line.substr(0, lastSpaceIndex).trim();
                                lines.push(line);
                                paragraph = paragraph.substr(lastSpaceIndex + 1).trim();
                        }
                }
